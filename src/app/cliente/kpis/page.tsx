...
const CHART_METRICS = [
  { key: 'active_contacts' as MetricKey,  label: 'Reuniones activas',      color: '#3b82f6' },
  { key: 'proposals_sent' as MetricKey,   label: 'Propuestas presentadas', color: '#10b981' },
]
...
export default function ClienteKpisPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: companyData }, { data: kpiData }] = await Promise.all([
        supabase.from('companies').select('name').eq('id', company_id).single(),
        supabase.from('kpis').select('*').eq('company_id', company_id).order('week_date', { ascending: true }),
      ])
  ...
  async function saveEntry() {
    ...
    if (data) {
      setEntries(prev =>
        [...prev, data].sort((a, b) => new Date(a.week_date).getTime() - new Date(b.week_date).getTime())
      )
      setActiveContacts(''); setMonitoredTenders(''); setProposalsSent(''); setPipelineValue(''); setNotes('')
      setShowForm(false)
    }
    setSaving(false)
  }
  ...
  return (
    <ClientShell companyName={companyName} active="kpis">
      <div className="px-8 py-8 max-w-4xl">
        <div className="mb-8 flex justify-between items-end">
          <h2 className="text-2xl font-bold">KPIs y métricas</h2>
          {!showForm && (
            <button
              onClick={() => { setWeekDate(new Date().toISOString().split('T')[0]); setShowForm(true) }}
        ...
        {/* Recharts for active_contacts and proposals_sent */}
        {entries.length > 1 && (
                  ...
                  <div className="h-20">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData}>
                        ...
                        />
                        <Line type="monotone" dataKey="value" stroke={m.color} strokeWidth={2} dot={false} activeDot={{ r: 3, fill: m.color }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={activeContacts} onChange={e => setActiveContacts(e.target.value)} placeholder="0"
                ...
                <div>
                  ...
                  <input type="number" value={monitoredTenders} onChange={e => setMonitoredTenders(e.target.value)} placeholder="0"
                    className="w-full bg-slate-800 text-white rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-blue-500 placeholder:text-slate-600"
                  />
                </div>
                <div>
                  <label className="text-slate-400 text-xs mb-1.5 block">Propuestas presentadas</label>
                  <input type="number" value={proposalsSent} onChange={e => setProposalsSent(e.target.value)} placeholder="0"
                    className="w-full bg-slate-800 text-white rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-blue-500 placeholder:text-slate-600"
                  />
                </div>
                <div>
                  <label className="text-slate-400 text-xs mb-1.5 block">Pipeline estimado (CLP)</label>
                  <input type="number" value={pipelineValue} onChange={e => setPipelineValue(e.target.value)} placeholder="0"
                    className="w-full bg-slate-800 text-white rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-blue-500 placeholder:text-slate-600"
                  />
                </div>
              </div>
